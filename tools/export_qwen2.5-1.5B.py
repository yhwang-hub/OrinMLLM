"""
This script exports Qwen2.5-1.5B-Instruct model to .bin format for KuiperLLama.

Usage:
    cd /home/lvf6/disk/wyh/KuiperLLama && \
    python tools/export_qwen2.5-1.5B.py Qwen2.5-1.5B.bin \
        --hf=/home/lvf6/disk/wyh/QwenModels/Qwen2.5-1.5B-Instruct/

Qwen2.5-1.5B-Instruct model configuration:
- hidden_size: 1536
...
def load_hf_model(model_path):
    """
    Load Qwen2.5-1.5B-Instruct model from HuggingFace format.
    ...
    # final classifier - 处理 tied embeddings 的情况
    if 'lm_head.weight' in hf_dict:
        model.output.weight = nn.Parameter(hf_dict['lm_head.weight'])
        print("Using separate lm_head.weight")
    else:
        # Qwen2.5-1.5B 使用 tied embeddings，lm_head 与 embed_tokens 共享权重
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Export Qwen2.5-1.5B-Instruct to bin format")
