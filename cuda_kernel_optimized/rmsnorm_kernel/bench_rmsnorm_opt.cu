...
// ============ Optimized kernels (same as in rmsnorm_kernel.cu) ============
...
// Optimized Pure FP16 kernel with 128-bit loads
template <int32_t BLOCK_DIM>
...
int main() {
  printf("=== RMSNorm Kernel Benchmark (Optimized) ===\n");
  ...
  printf("--- opt_row_rmsnorm_f32 (1 row, %d elements) ---\n", HIDDEN_SIZE);
  auto t = benchmark_kernel_ms([&](){ opt_row_rmsnorm_f32<128><<<1,128>>>(d_in_f32,d_wei_f32,d_out_f32,HIDDEN_SIZE,eps); }, WARMUP, ITERS);
  printf("  Time: %.3f us\n\n", t * 1000);

  for (int bs : batch_sizes) {
    printf("--- opt_row_rmsnorm_f32_dim (batch=%d, hidden=%d) ---\n", bs, HIDDEN_SIZE);
    t = benchmark_kernel_ms([&](){ opt_row_rmsnorm_f32_dim<128><<<bs,128>>>(d_in_f32,d_wei_f32,d_out_f32,bs,HIDDEN_SIZE,eps); }, WARMUP, ITERS);
    printf("  Time: %.3f us\n\n", t * 1000);
  }

  printf("--- opt_row_rmsnorm_pure_fp16 (1 row, %d elements) ---\n", HIDDEN_SIZE);
  t = benchmark_kernel_ms([&](){ opt_row_rmsnorm_pure_fp16<128><<<1,128>>>(d_in_fp16,d_wei_fp16,d_out_fp16,HIDDEN_SIZE,eps); }, WARMUP, ITERS);
  printf("  Time: %.3f us\n\n", t * 1000);
