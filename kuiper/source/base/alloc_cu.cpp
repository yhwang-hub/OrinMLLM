...
void* CUDADeviceAllocator::allocate(size_t byte_size) const {
  ...
  void* ptr = nullptr;
  // Check for any previous CUDA errors before cudaMalloc
  // cudaGetLastError() consumes and clears the error
  cudaError_t prev_error = cudaGetLastError();
  if (prev_error != cudaSuccess) {
    LOG(ERROR) << "Previous CUDA error before cudaMalloc: " 
               << cudaGetErrorString(prev_error);
  }
  ...
  if (cudaSuccess != state) {
    char buf[512];
    snprintf(buf, 512,
             "Error: CUDA error when allocating %lu bytes (%lu MB) memory! Error: %s",
             byte_size, byte_size >> 20, cudaGetErrorString(state));
    LOG(ERROR) << buf;
    return nullptr;
  }
