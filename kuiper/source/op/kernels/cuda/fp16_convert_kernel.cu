...
/**
 * Copy FP16 tensor from CPU to GPU, keeping FP16 format
 * Uses chunked transfer with pinned memory for efficiency with mmap'd data
 */
void fp16_cpu_to_fp16_gpu(const uint16_t* input_cpu, uint16_t* output_gpu, size_t size, cudaStream_t stream) {
    if (size == 0) return;
    
    // For very large tensors, process in chunks
    const size_t MAX_CHUNK_ELEMENTS = 128 * 1024 * 1024;  // 128M elements = 256MB
    size_t remaining = size;
    size_t offset = 0;
    ...
    while (remaining > 0) {
        ...
        // Copy FP16 data from staging memory to GPU
        if (stream) {
            err = cudaMemcpyAsync(output_gpu + offset, pinned_buffer, byte_size, cudaMemcpyHostToDevice, stream);
            cudaStreamSynchronize(stream);
        } else {
            err = cudaMemcpy(output_gpu + offset, pinned_buffer, byte_size, cudaMemcpyHostToDevice);
        }
