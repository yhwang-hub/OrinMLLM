...
std::string BpeEncodeLayer::decode(int32_t token_id) const {
  // Wrap single token in vector and delegate to multi-token decode
...
std::vector<int32_t> QwenEncodeLayer::encode(const std::string& sentence) const {
  CHECK(this->tiktoken_ != nullptr);
  // Qwen tokenizer doesn't need space->Ġ replacement
...
std::string QwenEncodeLayer::decode(const std::vector<int32_t>& token_ids) const {
  CHECK(this->tiktoken_ != nullptr);
  // Qwen tokenizer doesn't need Ġ->space replacement
