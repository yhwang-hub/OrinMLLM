#pragma once

#include <cstdint>
#include <string>
#include <glog/logging.h>

namespace common {

enum class ModelBufferType {
    kInputTokens = 0,
    kInputEmbeddings = 1,
    kOutputRMSNorm = 2,
    kKeyCache = 3,
    kValueCache = 4,
    kQuery = 5,
    kInputPos = 6,
    kScoreStorage = 7,
    kOutputMHA = 8,
    kAttnOutput = 9,
    kW1Output = 10,
    kW2Output = 11,
    kW3Output = 12,
    kFFNRMSNorm = 13,
    kForwardOutput = 15,
    kForwardOutputCPU = 16,

    kSinCache = 17,
    kCosCache = 18,
    
    // Fixed buffers for CUDA Graph optimization (decode phase)
    kTempKey = 19,        // Temporary key buffer with fixed address
    kTempValue = 20,      // Temporary value buffer with fixed address
    kInputPosGPU = 21,    // Position tensor on GPU for CUDA Graph (M-RoPE text position)
    kDecodeInput = 22,    // Fixed input buffer for decode phase (CUDA Graph)
    
    // Pinned memory buffers for async H2D/D2H transfers
    kInputPosPinned = 23, // Pinned memory for pos (enables true async H2D transfer)
    kArgmaxOutput = 24,   // Pre-allocated argmax output on GPU (avoids per-decode allocation)
    kArgmaxOutputPinned = 25, // Pinned memory for argmax result (enables true async D2H transfer)
    kArgmaxPinned = 25, // Alias for kArgmaxOutputPinned (for consistency)
    
    // Additional buffers for VL models
    kKVCachePosGPU = 26,  // KV cache position on GPU (for VL models with different RoPE/cache positions)
    kKVCachePosPinned = 27,  // Pinned memory for KV cache position
};

enum class DeviceType : uint8_t {
    kDeviceUnknown = 0,
    kDeviceCPU = 1,
    kDeviceCUDA = 2,
};

enum class DataType : uint8_t {
    kDataTypeUnknown = 0,
    kDataTypeFp16 = 1,
    kDataTypeFp32 = 2,
    kDataTypeInt8 = 3,
    kDataTypeInt32 = 4,
};

enum class ModelType : uint8_t {
    kModelTypeUnknown = 0,
    kModelTypeQwen2 = 1,
    kModelTypeQwen3 = 2,
    kModelTypeQwen3VL = 3,
    kModelTypeQwen35 = 4,
};

enum class StatusCode : uint8_t {
    kSuccess = 0,
    kFunctionUnImplement = 1,
    kPathNotValid = 2,
    kModelParseError = 3,
    kInternalError = 5,
    kKeyValueHasExist = 6,
    kInvalidArgument = 7,
};

enum class TokenizerType {
    kEncodeUnknown = -1,
    kEncodeSpe = 0,
    kEncodeBpe = 1,
};

enum class AttentionType : uint8_t {
    kAttentionMHA = 0,      // Standard Multi-Head Attention (materialized score matrix)
    kAttentionFlash1 = 1,   // FlashAttention v1 (online softmax, tiled K/V)
    kAttentionFlash2 = 2,   // FlashAttention v2 (reduced non-matmul FLOPs, better parallelism)
};

enum class MemcpyKind {
    kMemcpyCPU2CPU = 0,
    kMemcpyCPU2CUDA = 1,
    kMemcpyCUDA2CPU = 2,
    kMemcpyCUDA2CUDA = 3,
};

enum class MemoryType {
    kMemoryUnknown = 0,
    kMemoryCPU = 1,
    kMemoryCUDA = 2,
    kMemoryCPUPinned = 3,
    kMemoryCUDAZeroCopy = 4, // CUDA zero-copy memory (mapped to CPU address space)
};

inline const char* AttentionTypeName(AttentionType type) {
    switch (type) {
        case AttentionType::kAttentionMHA: return "MHA";
        case AttentionType::kAttentionFlash1: return "FlashAttention1";
        case AttentionType::kAttentionFlash2: return "FlashAttention2";
        default: return "Unknown";
    }
}

inline size_t DataTypeSize(DataType data_type) {
    if (data_type == DataType::kDataTypeFp32) {
        return sizeof(float);
    } else if (data_type == DataType::kDataTypeInt8) {
        return sizeof(int8_t);
    } else if (data_type == DataType::kDataTypeInt32) {
        return sizeof(int32_t);
    } else if (data_type == DataType::kDataTypeFp16) {
        return 2;  // sizeof(__half) = 2 bytes
    } else {
        return 0;
    }
}

class NoCopyable {
protected:
    NoCopyable() = default;
    ~NoCopyable() = default;
    NoCopyable(const NoCopyable&) = delete;
    NoCopyable& operator=(const NoCopyable&) = delete;
};

class Status {
public:
    Status(StatusCode code = StatusCode::kSuccess, std::string err_message = "");
    Status(const Status& other) = default;
    Status& operator=(const Status& other) = default;
    Status& operator=(StatusCode code);
    bool operator==(StatusCode code) const;
    bool operator!=(StatusCode code) const;
    operator StatusCode() const;
    operator bool() const;
    int32_t get_err_code() const;
    const std::string& get_err_msg() const;
    void set_err_msg(const std::string& err_msg);

private:
    StatusCode code_ = StatusCode::kSuccess;
    std::string err_message_;
};

#define STATUS_CHECK(call)                                                                 \
    do {                                                                                     \
        const Status& status = call;                                                     \
        if (!status) {                                                                         \
            const size_t buf_size = 512;                                                         \
            char buf[buf_size];                                                                  \
            snprintf(buf, buf_size - 1,                                                          \
                    "Infer error\n File:%s Line:%d\n Error code:%d\n Error msg:%s\n", __FILE__, \
                    __LINE__, int(status), status.get_err_msg().c_str());                       \
            LOG(FATAL) << buf;                                                                   \
        }                                                                                      \
    } while (0)

Status Success(const std::string& err_msg = "");
Status FunctionNotImplement(const std::string& err_msg = "");
Status PathNotValid(const std::string& err_msg = "");
Status ModelParseError(const std::string& err_msg = "");
Status InternalError(const std::string& err_msg = "");
Status KeyHasExits(const std::string& err_msg = "");
Status InvalidArgument(const std::string& err_msg = "");

std::ostream& operator<<(std::ostream& os, const Status& status);

}