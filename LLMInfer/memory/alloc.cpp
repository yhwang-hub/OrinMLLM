...
void DeviceAllocator::memcpy(const void* src_ptr, void* dest_ptr, size_t byte_size,
                             common::MemcpyKind memcpy_kind, void* stream, bool need_sync) const {
    ...
    if (memcpy_kind == common::MemcpyKind::kMemcpyCPU2CPU) {
        std::memcpy(dest_ptr, src_ptr, byte_size);
    } else if (memcpy_kind == common::MemcpyKind::kMemcpyCPU2CUDA) {
        if (!stream_) {
    ...
    } else if (memcpy_kind == common::MemcpyKind::kMemcpyCUDA2CPU) {
        if (!stream_) {
    ...
    } else if (memcpy_kind == common::MemcpyKind::kMemcpyCUDA2CUDA) {
        if (!stream_) {
...
void DeviceAllocator::memset_zero(void* ptr, size_t byte_size, void* stream,
                                  bool need_sync) {
    CHECK(device_type_ != common::DeviceType::kDeviceUnknown);
    if (device_type_ == common::DeviceType::kDeviceCPU) {
